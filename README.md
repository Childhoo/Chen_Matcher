...
## Experiments 
#OriNet 
2 different versions: OriNetFast, OriNetFast_Quarternion

Experiments image matching task: test the performance of image matching for image pairs containing planar object.

Experiments on image orientation task:
1) HAOH (brown brown) with Hardnet
2) HAOH (graz brown) with Hardnet
3）HAOW (graz brown) with WeMNet