...
def batch_eig2x2(A):
    ...
    #for case where delta1<0, it just assigns 1000 and 0.0001 to it
    # unable to give a ratio for real value?
...
#    l2 =  (trace - delta) / 2.0
    ...
    #seeking for svd decomposition 
    #https://lucidar.me/en/mathematics/singular-value-decomposition-of-a-2x2-matrix/
#    s1 = A[:,0,0]* A[:,0,0] + A[:,1,0]* A[:,1,0] + A[:,0,1]* A[:,0,1] + A[:,1,1]* A[:,1,1]
#    s21 = (A[:,0,0]* A[:,0,0] + A[:,1,0]* A[:,1,0] + A[:,0,1]* A[:,0,1] + A[:,1,1]* A[:,1,1])*(A[:,0,0]* A[:,0,0] + A[:,1,0]* A[:,1,0] + A[:,0,1]* A[:,0,1] + A[:,1,1]* A[:,1,1])
#    s22 =  4*(A[:,0,0]* A[:,1,0] + A[:,0,1]* A[:,1,1])*(A[:,0,0]* A[:,1,0] + A[:,0,1]* A[:,1,1])
#    s2 = torch.sqrt(s21+s22)
#    l1 = torch.sqrt((s1+s2)/2.0 + 1e-12)
#    l2 = torch.sqrt(torch.abs((s1-s2)/2.0 + 1e-12))
    return l1,l2
