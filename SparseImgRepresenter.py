...
class ScaleSpaceAffinePatchExtractor(nn.Module):
    ...
    def getAffineShape(self, final_resp, LAFs, final_pyr_idxs, final_level_idxs, do_scale_check, num_features = 0):
        pe_time = 0
        affnet_time = 0
        ...
        for i in range(self.num_Baum_iters):
            ...
            if i != self.num_Baum_iters - 1:
                ...
                #for the new iteration, the matrix begins to be similar form than last iteration
        l1,l2 = batch_eig2x2(base_A)
        ratio = torch.abs(l1 / (l2 + 1e-8))
        print('ratio:',ratio) #output the ratio of two axes in affine region
        #do scale check or not
        if do_scale_check:
            idxs_mask =  ((ratio < 6.0) * (ratio > (1./6.))) * checkTouchBoundary(new_LAFs)
        else:
            idxs_mask =  checkTouchBoundary(new_LAFs)
        num_survived = idxs_mask.float().sum()
        #if (num_features > 0) and (num_survived.data[0] > num_features):
        if (num_features > 0) and (num_survived > num_features):
        ...
        print('LAFs shape:',LAFs.shape)
        print('base_A shape:',base_A.shape)
    ...
    def getOrientation(self, LAFs, final_pyr_idxs, final_level_idxs):
        ...
        for i in range(max_iters):
            ...
            if i != max_iters:
                patches_small = extract_patches_from_pyramid_with_inv_index(self.scale_pyr, pyr_inv_idxs, LAFs, PS = self.OriNet.PS)        
        return LAFs
    #extract patches from detected LAFs in scale space pyramid
    def extract_patches_from_pyr(self, dLAFs, PS = 41):
        ...
        patches = extract_patches_from_pyramid_with_inv_index(self.scale_pyr,
                                                      ...
                                                      PS = PS)
        return patches
    #run the scale check based on the ratio of eigenvalues for the transformation matrix A or not
    def forward(self,x, do_ori = False, do_scale_check = True):
        ### Detection
        t = time.time()
...
#        print (time.time() - t, 'detection multiscale')
        ...
        if self.num_Baum_iters > 0:
            responses, LAFs, final_pyr_idxs, final_level_idxs  = self.getAffineShape(responses, LAFs, final_pyr_idxs, final_level_idxs, do_scale_check, self.num)
