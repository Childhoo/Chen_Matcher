...
#        datasets_path = [os.path.join(datasets_path, dataset) for dataset in os.listdir(datasets_path)]
        datasets_path = glob.glob(datasets_path + "/*.pt")
        start = True
...
#this one only generate patch dataset, not pairs for affnet and orinet training
class PatchesDatasetLoader(data.Dataset):
    def __init__(self, datasets_path, train = True, transform = None, batch_size = None, n_triplets = 5000000, fliprot = False, *arg, **kw):
        super(PatchesDatasetLoader, self).__init__()
#        datasets_path = [os.path.join(datasets_path, dataset) for dataset in os.listdir(datasets_path)]
        datasets_path = glob.glob(datasets_path + "/*.pt")
        start = True
        for dataset_p in datasets_path:
    ...
    #here generating pairs is not actually needed, only generating single patches
    def generate_pairs(self, labels, num_triplets, batch_size):
            def create_indices(_labels):
                inds = dict()
                for idx, ind in enumerate(_labels):
            ...
            for x in tqdm(range(num_triplets)):
                ...
                try:
                    ...
                    print (indices.keys())
                    sys.exit(0)
                if len(indices[c1]) == 1: #only one indices, only choose one
                    n1 = 0
                else:
