...
#        angle = angle%(2*math.pi) #convert to 0 - 2pi to see what happened
        if return_rot_matrix:
            return get_rotation_matrix(angle)
        return angle

#sample bad geometric transformation in descriptor learning
class Bad_GeoSampler(nn.Module):
    def __init__(self, PS = 16):
        super(Bad_GeoSampler, self).__init__()
        self.features = nn.Sequential(
            nn.Conv2d(1, 16, kernel_size=3, padding=1, bias = False),
            nn.BatchNorm2d(16, affine=False),
            nn.ReLU(),
            ...
            nn.ReLU(),
            nn.Dropout(0.25),
            nn.Conv2d(64, 6, kernel_size=int(PS/4), stride=1,padding=1, bias = True),
            nn.Tanh(),
            nn.AdaptiveAvgPool2d(1)
        )
        self.PS = PS
    ...
    def weights_init(self,m):
        ...
        return
    def forward(self, input, return_rot_matrix = True):
        xy = self.features(self.input_norm(input)).view(-1,6) 
        full_psi = torch.atan2(xy[:,0] + 1e-8, xy[:,1]+1e-8)
        full_phi = torch.atan2(xy[:,2] + 1e-8, xy[:,3]+1e-8)
        
        #for the phi angle, -22.5 - + 22.5 degree
        # rotation -30 + 30 degree
        rot_longi = get_rotation_matrix(0.125*full_phi)
        rot_inplane = get_rotation_matrix(0.166*full_psi)
        
        four_theta = torch.atan2(xy[:,4] + 1e-8, xy[:,5]+1e-8)
        four_theta = torch.remainder(four_theta, 2.0*np.pi)
        # 0.125 pi = 45 degree, maximum allowable tilt to 45 degree
        # 1/sqrt(2) TO sqrt(2) scale change
        tilt = 1/(torch.cos(0.125*four_theta) + 1e-10)
        ...
        if xy.is_cuda:
            ...
            rot_longi = rot_longi.cuda()
            rot_inplane = rot_inplane.cuda()
        
        return torch.bmm(rot_inplane, torch.bmm(tilt_matrix, rot_longi))


#an estimator of full affine transformations
class Full_Affine_Estimator_Chen(nn.Module):
    def __init__(self, PS = 16):
        super(Full_Affine_Estimator_Chen, self).__init__()
        self.features = nn.Sequential(
            nn.Conv2d(1, 16, kernel_size=3, padding=1, bias = False),
            nn.BatchNorm2d(16, affine=False),
            nn.ReLU(),
    ...
    def forward(self, input, return_rot_matrix = True):
        ...
        #for the phi angle, -22.5 - + 22.5 degree
        # rotation -30 + 30 degree
        full_psi = torch.remainder(full_psi, 2.0*np.pi)
        full_phi = torch.remainder(full_phi, 2.0*np.pi)
        
        rot_longi = get_rotation_matrix(0.50*full_phi)
        rot_inplane = get_rotation_matrix(full_psi)
        
        four_theta = torch.atan2(xy[:,4] + 1e-8, xy[:,5]+1e-8)
        four_theta = torch.remainder(four_theta, 2.0*np.pi)
        # 0.125 * 2pi = 45 degree, maximum allowable tilt to 45 degree
        # 1/sqrt(2) TO sqrt(2) scale change
        
        # 0.2083*four_theta [76 degree of latitue] tilt = 4.02
        tilt = 1/(torch.cos(0.22*four_theta) + 1e-10)
...
class Full_Affine_Estimator_Chen_Quarternion(nn.Module):
    def __init__(self, PS = 16):
        super(Full_Affine_Estimator_Chen, self).__init__()
        self.features = nn.Sequential(
            nn.Conv2d(1, 16, kernel_size=3, padding=1, bias = False),
    ...
    def forward(self, input, return_rot_matrix = True):
        ...
        xy_all = self.features(self.input_norm(input)).view(-1,6) 
        xy = xy_all[:,0:2]
        #use quaternion to handle rotation matrix here
        #first normalize the two output units to make it x**2 + y**2 = 1
        xy_square = torch.sqrt((xy**2).sum(dim=1) + 1e-8)
        qua = xy/(xy_square.view(xy.size(0),1) + 1e-8)
        rot00 = 1-2*qua[:,1]**2
        rot00 = rot00.view(-1, 1, 1)
        rot01 = -2*qua[:,1]*qua[:,0]
        rot01 = rot01.view(-1, 1, 1)                
        A1_x = torch.cat([rot00, rot01], dim = 2)
        A2_x = torch.cat([-rot01, rot00], dim = 2)
        rot_inplane = torch.cat([A1_x,A2_x], dim = 1)
...
#        angle = torch.atan2(xy[:,0] + 1e-8, xy[:,1]+1e-8);
        angle = torch.atan2(2*qua[:,0]*qua[:,1] + 1e-8, 1 - 2*(qua[:,1])**2 + 1e-8);
        if return_rot_matrix:
#            rot_matrix = torch.eye(2).view(1,2,2).repeat(xy.size(0),1,1)
#            rot_matrix[:,0,0] = 1-2*qua[:,1]**2
#            rot_matrix[:,0,1] = -2*qua[:,1]*qua[:,0]
#            rot_matrix[:,1,0] = -rot_matrix[:,0,1]
#            rot_matrix[:,1,1] = rot_matrix[:,0,0]
            
#            angle_in_radians = angle.view(-1, 1, 1)
            rot00 = 1-2*qua[:,1]**2
            rot00 = rot00.view(-1, 1, 1)
            ...
            A1_x = torch.cat([rot00, rot01], dim = 2)
            A2_x = torch.cat([-rot01, rot00], dim = 2)
            rot_matrix = torch.cat([A1_x,A2_x], dim = 1)
...
#use quaternions to represent the rotation, not the Euler rotation matrix
class OriNetFast_Quaternion(nn.Module):
    def __init__(self, PS = 16):
        super(OriNetFast_Quaternion, self).__init__()
        self.features = nn.Sequential(
            nn.Conv2d(1, 16, kernel_size=3, padding=1, bias = False),
            nn.BatchNorm2d(16, affine=False),
            nn.ReLU(),
...
class AffNetFast_Nodropout(nn.Module):
    def __init__(self, PS = 32):
        super(AffNetFast_Nodropout, self).__init__()
        self.features = nn.Sequential(
            nn.Conv2d(1, 16, kernel_size=3, padding=1, bias = False),
            nn.BatchNorm2d(16, affine=False),
            nn.ReLU(),
...
#        tilt = 1/(torch.cos(0.25*four_lati) + 1e-10)
        tilt = 1/(torch.cos(0.21*four_lati) + 1e-10)
...
class AffNetFast4RotNosc_ResidualForm(nn.Module):
    def __init__(self, PS = 32):
        super(AffNetFast4RotNosc_ResidualForm, self).__init__()
        self.features = nn.Sequential(
            nn.Conv2d(1, 16, kernel_size=3, padding=1, bias = False),
            nn.BatchNorm2d(16, affine=False),
            nn.ReLU(),
    ...
    def forward(self, input, return_A_matrix = False):
        ...
        a2 = torch.cat([xy[:,2].contiguous().view(-1,1,1), 1.0 + xy[:,3].contiguous().view(-1,1,1)], dim = 2).contiguous()
        
        A = torch.cat([a1,a2], dim = 1).contiguous()
#        A = self.features(self.input_norm(input)).view(-1,2,2).contiguous()
        scale =  torch.sqrt(torch.abs(A[:,0,0]*A[:,1,1] - A[:,1,0]*A[:,0,1] + 1e-10))
        return A / (scale.view(-1,1,1).repeat(1,2,2) + 1e-8)


class AffNetFast4RotNosc_LowerDropout(nn.Module):
    def __init__(self, PS = 32):
        super(AffNetFast4RotNosc_LowerDropout, self).__init__()
        self.features = nn.Sequential(
            nn.Conv2d(1, 16, kernel_size=3, padding=1, bias = False),
            nn.BatchNorm2d(16, affine=False),
            nn.ReLU(),
...
#        angle = torch.atan2(x[:,1], x[:,2]+1e-8);
        angle = torch.atan2(x[:,1]+1e-8, x[:,2]+1e-8); #corrected by Lin Chen
        rot = get_rotation_matrix(angle)
