...
#simulate angle and return the angle_mag returned
def get_random_rotation_LAFs_withsimanglereturn(patches, angle_mag = math.pi):
    rot_LAFs = Variable(torch.FloatTensor([[0.5, 0, 0.5],[0, 0.5, 0.5]]).unsqueeze(0).repeat(patches.size(0),1,1));
    phi  = (Variable(2.0 * torch.rand(patches.size(0)) - 1.0) ).view(-1,1,1)
    if patches.is_cuda:
        rot_LAFs = rot_LAFs.cuda()
    ...
    inv_rotmat = get_rotation_matrix(-angle_mag * phi)
    rot_LAFs[:,0:2,0:2]  = torch.bmm(rotmat, rot_LAFs[:,0:2,0:2]);
    return rot_LAFs, inv_rotmat, angle_mag*phi

def get_random_rotation_LAFs_forDescDistanAnalyse(patches, angle_mag = math.pi):
    rot_LAFs = Variable(torch.FloatTensor([[0.71, 0, 0.5],[0, 0.71, 0.5]]).unsqueeze(0).repeat(patches.size(0),1,1));
    phi  = (Variable(2.0 * torch.rand(patches.size(0)) - 1.0) ).view(-1,1,1)
    if patches.is_cuda:
        rot_LAFs = rot_LAFs.cuda()
        phi = phi.cuda()
...
def get_random_norm_affine_LAFs(patches, max_tilt = 1.0):
    ...
    if patches.is_cuda:
        inv_TA = inv_TA.cuda()
    TA = TA.cpu() #cuda for small matrix are super slow
