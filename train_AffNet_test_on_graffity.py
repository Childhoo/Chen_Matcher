...
np_reshape = lambda x: np.reshape(x, (96, 96, 1))
...
#        geom_dist = torch.sqrt(((out_a_aff_back - out_p_aff_back)**2 ).view(-1,4).sum(dim=1) + 1e-8).mean()
        geom_dist_a = torch.sqrt(((out_a_aff - inv_TA_a)**2).view(-1,4).sum(dim=1) + 1e-8).mean()
        geom_dist_p = torch.sqrt(((out_p_aff - inv_TA_p)**2).view(-1,4).sum(dim=1) + 1e-8).mean()
...
#        loss = loss + 0.5*geom_dist # a good georefrence should be attained for a perfect match, at least to some level
#        loss = loss + 0.1*geom_dist_a + 0.1*geom_dist_p # a good georefrence should be attained for a perfect match, at least to some level
        loss = loss
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
#        test(model, epoch)
        train_loader, test_loader = create_loaders() #addde on 13.09.2019
    return 0
